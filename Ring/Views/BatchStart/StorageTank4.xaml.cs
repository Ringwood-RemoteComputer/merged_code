...
namespace Ring.Views.BatchStart
{
    ...
    public partial class StorageTank4 : Window
    {
        ...
        private const int TANK_INDEX = 3; // Tanks[3] for Storage Tank 4
        ...
        public StorageTank4()
        {
            ...
            this.Loaded += StorageTank4_Loaded;
            this.Closing += StorageTank4_Closing;
        }

        private void StorageTank4_Loaded(object sender, RoutedEventArgs e)
        {
        ...
        private void StorageTank4_Closing(object sender, System.ComponentModel.CancelEventArgs e)
        {
        ...
        private void InitializePlcTags()
        {
            try
            {
                if (!_simulationMode)
                {
                    ...
                    Console.WriteLine($"[StorageTank4] PLC tags initialized for Tanks[{TANK_INDEX}]");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[StorageTank4] Error initializing PLC tags: {ex.Message}");
            }
        }
        ...
        private void ReadFromPlc()
        {
            ...
            {
                Console.WriteLine($"[StorageTank4] Error reading from PLC: {ex.Message}");
            }
            finally
            {
        ...
        private void WriteToPlc(string tagName, int value, PlcTagWriter writer)
        {
            ...
            try
            {
                ...
                if (success)
                {
                    Console.WriteLine($"[StorageTank4] ? Wrote {tagName} = {value}");
                }
                else
                {
                    Console.WriteLine($"[StorageTank4] ? Failed to write {tagName} = {value}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[StorageTank4] Error writing {tagName}: {ex.Message}");
            }
        }
        ...
        private void FormulaListBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (FormulaListBox.SelectedItem is ListBoxItem selectedItem)
            {
                ...
                Console.WriteLine($"[StorageTank4] Formula selected: {selectedItem.Content} (value: {formulaNumber})");
            ...
            if (sender is RadioButton radioButton)
            {
                ...
                if (radioButton.GroupName == "BatchType")
                {
                    ...
                    if (index >= 0 && index <= 2)
                    {
                        ...
                        Console.WriteLine($"[StorageTank4] Batch Start Type changed to: {radioButton.Content} (value: {index})");
                    }
                }
                ...
                else if (radioButton.GroupName == "SplitA")
                {
                    ...
                    if (index >= 0 && index <= 3)
                    {
                        ...
                        Console.WriteLine($"[StorageTank4] Splitting Type A changed to: {radioButton.Content} (value: {index})");
                    }
                }
                ...
                else if (radioButton.GroupName == "SplitB")
                {
                    ...
                    if (index >= 0 && index <= 3)
                    {
                        ...
                        Console.WriteLine($"[StorageTank4] Splitting Type B changed to: {radioButton.Content} (value: {index})");
                    }
                }
            }
