...
{
    /// <summary>
    /// Interaction logic for StorageTank2.xaml
    /// </summary>
    public partial class StorageTank2 : Window
    {
        ...
        private const int TANK_INDEX = 1; // Tanks[1] for Storage Tank 2
        ...
        public StorageTank2()
        {
            ...
            this.Loaded += StorageTank2_Loaded;
            this.Closing += StorageTank2_Closing;
        }

        private void StorageTank2_Loaded(object sender, RoutedEventArgs e)
        {
        ...
        private void StorageTank2_Closing(object sender, System.ComponentModel.CancelEventArgs e)
        {
        ...
        private void InitializePlcTags()
        {
            try
            {
                if (!_simulationMode)
                {
                    ...
                    Console.WriteLine($"[StorageTank2] PLC tags initialized for Tanks[{TANK_INDEX}]");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[StorageTank2] Error initializing PLC tags: {ex.Message}");
            }
        }
        ...
        private void ReadFromPlc()
        {
            ...
            {
                Console.WriteLine($"[StorageTank2] Error reading from PLC: {ex.Message}");
            }
            finally
            {
        ...
        private void WriteToPlc(string tagName, int value, PlcTagWriter writer)
        {
            ...
            try
            {
                ...
                if (success)
                {
                    Console.WriteLine($"[StorageTank2] ✓ Wrote {tagName} = {value}");
                }
                else
                {
                    Console.WriteLine($"[StorageTank2] ✗ Failed to write {tagName} = {value}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[StorageTank2] Error writing {tagName}: {ex.Message}");
            }
        }
        ...
        private void FormulaListBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (FormulaListBox.SelectedItem is ListBoxItem selectedItem)
            {
                ...
                Console.WriteLine($"[StorageTank2] Formula selected: {selectedItem.Content} (value: {formulaNumber})");
            ...
            if (sender is RadioButton radioButton)
            {
                ...
                if (radioButton.GroupName == "BatchType")
                {
                    ...
                    if (index >= 0 && index <= 2)
                    {
                        ...
                        Console.WriteLine($"[StorageTank2] Batch Start Type changed to: {radioButton.Content} (value: {index})");
                    }
                }
                ...
                else if (radioButton.GroupName == "SplitA")
                {
                    ...
                    if (index >= 0 && index <= 3)
                    {
                        ...
                        Console.WriteLine($"[StorageTank2] Splitting Type A changed to: {radioButton.Content} (value: {index})");
                    }
                }
                ...
                else if (radioButton.GroupName == "SplitB")
                {
                    ...
                    if (index >= 0 && index <= 3)
                    {
                        ...
                        Console.WriteLine($"[StorageTank2] Splitting Type B changed to: {radioButton.Content} (value: {index})");
                    }
                }
            }
